...
public class VayraDamagedAutomation extends BaseHullMod {
    ...
    public void applyEffectsBeforeShipCreation(HullSize hullSize, MutableShipStatsAPI stats, String id) {
        ...
        float crPenalty = calculateCRPenalty(effect);
        float minCrewMult = calculateMinCrewMultiplier(effect);
    ...
    private float calculateCRPenalty(float baseEffect) {
    ...
    public String getDescriptionParam(int index, HullSize hullSize, ShipAPI ship) {
        ...
        float crPenalty = calculateCRPenalty(effect);
        float minCrewMalus = calculateMinCrewMalus(effect);
    ...
    private float calculateMinCrewMultiplier(float baseEffect) {
        // Basically, min crew multiplier is going to be 1 + MinCrewMalus
        float retVal;
        float malus = calculateMinCrewMalus(baseEffect);
        retVal = 1f + malus;

        return retVal;
    }

    private float calculateMinCrewMalus(float baseEffect) {
        // Since having "rugged" is already implicitly a part of baseEffect, meaning it will come in as 0.5
        // instead of 1.0, the penalty factor will also end up being 0.5 so we don't need to check for rugged
